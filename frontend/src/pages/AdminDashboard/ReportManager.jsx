...
export default function ReportManager() {
  ...
  const exportToPDF = () => {
    ...
    const title = `GIT Maintenance Report - Generated ${formatDate(new Date())}`;
    ...
    const tableColumn = ["Indent ID", "ISR No", "Date", "Generated By", "Assigned Department", "Status", "Description"];
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider whitespace-nowrap">ISR No</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {reportData.length > 0 ? reportData.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-700 whitespace-nowrap">{row.isrNo || '-'}</td>
