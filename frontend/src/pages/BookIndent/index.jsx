...
export default function BookIndentPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.bookEdition.trim()) return 'Book Edition is required';
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider">HOD Remark</th>
                ...
                {isLoading ? (
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-500">Edition: {item.bookEdition || '-'} &bull; ISBN: {item.isbn || '-'}</div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 overflow-y-auto">
          <div className="w-full max-w-2xl rounded-2xl bg-white shadow-xl my-8">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Book Edition <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                    value={formData.bookEdition}
                    onChange={handleChange('bookEdition')}
                    placeholder="e.g. 3rd Edition"
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">ISBN <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    required
                    value={formData.isbn}
